...
private:
	...
	bool in_array (Point const & p) const;
	array_size_t edge_hash (Point p, dir_id_t dir);
	void add_shift (value_t new_shift);
	void enable_point (Point const & p);
	void erase_single_point (Point const & p, dir_id_t dir_id);
	void erase_double_point (Point const & p, dir_id_t dir_id);
	void insert_single_edge (Point const & p, dir_id_t dir_id);
	void insert_double_edge (Point const & p, dir_id_t dir_id);
	void insert_point (Point const & p);
